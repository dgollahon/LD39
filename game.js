...
class Level {
    ...
    rightCollision(leftX, rightX, y) {
        let block = this.horizontalTiles(leftX, rightX, y).filter(tile => tile.isBlocking() && tile.upperBlocking)[0];
        if (block) {
            return block.x - TILE_SIZE / 2;
        }
        else {
    ...
    topCollision(topY, bottomY, x) {
        let blockingTiles = this.verticalTiles(topY, bottomY, x).filter(tile => tile.upperBlocking);
        let block = blockingTiles[blockingTiles.length - 1];
        if (block) {
