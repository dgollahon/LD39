    ...
    constructor() {
        ...
        this.tiles = Level.data.map(row => {
            ...
            for (let col = 0; col < columns; ++col) {
                ...
                columnTiles.push(row[col] === "X" ? new Tile(tileX, tileY) : new EmptyTile(tileX, tileY));
            }
            tileY += TILE_SIZE;
            return columnTiles;
        });
...
class Player {
    ...
    draw() {
        ...
        if (this.faceRight) {
            // context.scale(-1, 1);
            context.drawImage(PLAYER_IMAGE, TILE_SIZE * Math.floor(this.frame / 4), 0, TILE_SIZE, TILE_SIZE, this.x - HALF_TILE, this.y - HALF_TILE, TILE_SIZE, TILE_SIZE);
        }
        else {
            context.translate(this.x - HALF_TILE, this.y - HALF_TILE);
            context.scale(-1, 1);
            context.drawImage(PLAYER_IMAGE, TILE_SIZE * Math.floor(this.frame / 4), 0, TILE_SIZE, TILE_SIZE, -TILE_SIZE, 0, TILE_SIZE, TILE_SIZE);
        }
        context.restore();
    }
