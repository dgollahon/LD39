...
class Level {
  ...
  public draw(context: CanvasRenderingContext2D, player: Player) {
    let y = 0;
    while (y < HEIGHT) {
      const row = this.horizontalTiles(
        Math.max(player.x - WIDTH / 2, 0),
        Math.max(player.x + WIDTH / 2 + TILE_SIZE, WIDTH),
        y
      );
...
class Enemy {
  ...
  public draw(context: CanvasRenderingContext2D, timestamp: number) {
